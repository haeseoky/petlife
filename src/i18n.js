import { ref, watch } from 'vue'

const messages = {
  ko: {
    appTitle: '🐾 PetLife',
    heroEmoji: '🐾',
    heroTitle: '반려동물 사주 보기',
    heroDesc: '강아지·고양이의 사주(四柱)를 재미있게 분석해드려요!',
    badges: ['사주 분석', '오행 균형', '성격 파악', '운세 체크'],
    petType: '반려동물 종류',
    dog: '강아지',
    cat: '고양이',
    nameLabel: '이름',
    namePlaceholder: '예: 초코',
    catPlaceholder: '예: 나비',
    breedLabel: '품종',
    breedDefault: '품종 선택',
    birthLabel: '생년월일',
    hourLabel: '태어난 시각',
    submitBtn: '사주 보기',
    fourPillars: '사주명식 (四柱)',
    year: '년(年)',
    month: '월(月)',
    day: '일(日)',
    hour: '시(時)',
    ilju: '일주',
    personality: '성격 분석',
    traits: '특징',
    fiveElements: '오행(五行) 분포',
    missingElements: '부족한 오행',
    walkStyle: '산책 스타일',
    catPlayStyle: '놀이 & 활동 스타일',
    foodPref: '식성 & 추천 식단',
    healthTip: '건강 관리 팁',
    compatible: '잘 맞는 일주',
    todayLuck: '오늘의 운세',
    todayIlju: '오늘 일주',
    monthlyLuck: '이달의 운세',
    monthlyEnergy: '이달의 기운',
    careTips: '맞춤 케어 팁',
    shareBtn: '결과 공유하기',
    printBtn: '저장/인쇄',
    imageBtn: '이미지 저장',
    saving: '저장 중...',
    resetBtn: '다시 보기',
    copied: '클립보드에 복사되었습니다',
    historyTitle: '저장된 사주 기록',
    historyDesc: '최대 20개까지 자동 저장됩니다',
    compatTitle: '반려동물 궁합',
    compatDesc: '다른 반려동물과의 궁합을 확인해보세요!',
    compatNameLabel: '이름',
    compatBirthLabel: '생년월일',
    compatHourLabel: '태어난 시',
    compatBtn: '궁합 보기',
    compatChecking: '확인 중...',
    compatRetry: '다시 확인',
    missingWood: '산책과 자연 접촉이 좋아요! 공원 산책을 자주 시켜주세요.',
    missingFire: '따뜻한 햇살을 좋아해요. 일광욕 시간을 충분히 주세요.',
    missingEarth: '안정적인 환경이 중요해요. 집을 편안하게 꾸며주세요.',
    missingMetal: '깔끔한 그루밍이 중요해요. 정기적으로 미용해주세요.',
    missingWater: '물과 관련된 활동을 좋아할 수 있어요. 수영을 시도해보세요!',
    balanced: '오행이 골고루 갖춰져 있어 아주 균형 잡힌 강아지예요!',
    footer: '© 2026 PetLife — 반려동물과 함께하는 행복한 삶 🐶',
    darkMode: '다크 모드',
    lightMode: '라이트 모드',
    records: '기록',
    close: '닫기',
    yearSuffix: '년',
    monthSuffix: '월',
    daySuffix: '일',
    hourSuffix: '시',
    zodiacSuffix: '띠',
    of: '의 사주',
    resultOf: '의 사주 결과',
    saveProfile: '내 반려동물 프로필 저장',
    savedProfiles: '저장된 프로필',
    deleteProfile: '삭제',
    petTypeTitle: '반려동물 성격 유형',
    petTypeDesc: '사주의 오행 분포를 기반으로 분석한 성격 유형입니다.',
    petMBTITitle: '🐾 반려동물 MBTI 성격 유형',
    petMBTISubtitle: '사주 오행으로 분석한 우리 아이 성격',
    petMBTIStrengths: '주요 강점',
    petMBTIGrowth: '성장 포인트',
    petMBTIDisclaimer: '본 결과는 반려동물의 사주 기운을 기반으로 한 재미로 보는 성격 유형입니다.',
    personalityE: '활동적 E',
    personalityI: '차분함 I',
    personalityS: '현실적 S',
    personalityN: '상상력 N',
    personalityT: '논리적 T',
    personalityF: '감성적 F',
    personalityJ: '계획적 J',
    personalityP: '자율적 P',
    elementRadarTitle: '🔮 오행 레이더',
    elementRadarDesc: '반려동물의 오행 기운 분포를 한눈에 확인하세요',
    luckyNameTitle: '럭키 네임 추천',
    luckyNameDesc: '반려동물에게 좋은 기운을 주는 이름들이에요',
    refreshBtn: '다시 뽑기',
    meaningLabel: '의미',
    tarotTitle: '🐾 오늘의 타로',
    tarotInstruction: '마음에 드는 카드를 골라보세요',
    tarotRedraw: '다시 뽑기',
    tarotSharePrefix: '타로 카드',
    fengshuiTitle: '🧭 행운의 방위 & 인테리어',
    fengshuiDesc: '반려동물의 오행에 맞는 공간 배치로 좋은 기운을 더해보세요!',
    fengshuiBedDir: '침상 방향',
    fengshuiFoodDir: '식사 자리 방향',
    fengshuiColors: '추천 색상',
    fengshuiInterior: '인테리어 포인트',
    fengshuiTipsTitle: '맞춤 공간 팁',
    fengshuiBoostTitle: '부족한 오행 보완하기',
    fengshuiBoostDesc: '부족한 기운을 채우면 더 건강하고 행복한 생활이 가능해요.',
    balanceTitle: '🐾 반려동물 밸런스 게임',
    balanceDesc: '우리 아이의 취향을 알아보고 성향을 분석해보세요!',
    balanceStart: '게임 시작하기',
    balanceNext: '다음 질문',
    balanceResult: '결과 확인하기',
    balanceRetry: '다시 하기',
    weeklyHoroscope: '📅 이번 주 운세',
    weeklyHoroscopeDesc: '이번 주 7일간의 반려동물 운세를 확인해보세요!',
    weeklyScore: '운세 점수',
    todayHighlight: '오늘',
    balanceShare: '게임 결과 공유',
    balanceStat: '선택 통계',
    balanceTraitPrefix: '우리 아이는 ',
    balanceTraitSuffix: ' 성향이에요!',
    balanceQ1A: '새 장난감', balanceQ1B: '좋아하는 간식',
    balanceQ2A: '공원 산책', balanceQ2B: '집에서 뒹굴기',
    balanceQ3A: '꽉 안아주기', balanceQ3B: '옆에 조용히 앉기',
    balanceQ4A: '새로운 친구', balanceQ4B: '보호자와 단둘이',
    balanceQ5A: '공놀이', balanceQ5B: '밧줄 당기기',
    balanceQ6A: '햇살 좋은 창가', balanceQ6B: '그늘진 소파',
    balanceQ7A: '시원한 목욕', balanceQ7B: '깔끔한 발톱 깎기',
    balanceQ8A: '카시트로 드라이브', balanceQ8B: '가방에 들어가 산책',
    balanceQ9A: '복슬복슬 침대', balanceQ9B: '시원한 바닥',
    balanceQ10A: '달콤한 고구마', balanceQ10B: '든든한 닭가슴살',
    balanceQ11A: '장거리 여행', balanceQ11B: '집 근처 탐험',
    balanceQ12A: '삑삑이 장난감', balanceQ12B: '노즈워크 매트',
    balanceQ13A: '부드러운 빗질', balanceQ13B: '깨끗한 이빨 닦기',
    balanceQ14A: '상쾌한 아침 산책', balanceQ14B: '차분한 저녁 산책',
    balanceQ15A: '흥미로운 창밖 구경', balanceQ15B: '꿀같은 낮잠 자기',
    balanceQ16A: '물어오기 놀이', balanceQ16B: '숨바꼭질',
    balanceQ17A: '싱싱한 풀/캣닢', balanceQ17B: '바삭한 트릿',
    balanceQ18A: '높은 곳 정복', balanceQ18B: '아늑한 상자 속',
    balanceQ19A: '사람 음식 탐내기', balanceQ19B: '전용 간식 기다리기',
    balanceQ20A: '낯선 사람에게 꼬리치기', balanceQ20B: '보호자 뒤에 숨기',
    wishCouponTitle: '🐾 반려동물 소원권',
    wishCouponDesc: '우리 아이의 성향에 딱 맞는 특별한 소원권을 드려요!',
    wishCouponRegenerate: '새로운 소원권 뽑기',
    wishCouponUsed: '사용 완료',
    wishCouponUnused: '사용 전',
    wishCouponDate: '사용 일자',
    wishCouponPlaceholder: '____년 __월 __일',
    traitActive: '에너지 넘치는 활동가',
    traitRelaxed: '평화로운 휴식가',
    traitSocial: '사교적인 인기쟁이',
    traitIndependent: '당당한 독립가',
    traitSensitive: '섬세한 감성파',
    gachaTitle: '🐾 오늘의 운세 캡슐',
    gachaDesc: '오늘 우리 아이의 운세를 캡슐에서 뽑아보세요!',
    gachaBtn: '캡슐 뽑기',
    gachaDrawing: '캡슐이 나오는 중...',
    gachaResultTitle: '오늘의 행운 카드',
    gachaAffection: '애정운',
    gachaHealth: '건강운',
    gachaPlay: '놀이운',
    gachaAppetite: '식욕운',
    gachaDailyLimit: '운세는 하루에 한 번만 뽑을 수 있어요.',
    moodTrackerTitle: '오늘의 기분',
    moodTrackerDesc: '반려동물의 기분을 기록해보세요',
    moodStreak: '{n}일 연속 기록 중',
    moodHappy: '행복',
    moodSleepy: '졸림',
    moodPlayful: '놀고싶음',
    moodHungry: '배고픔',
    moodAnxious: '불안',
    moodNotLogged: '미기록',
    summerCareTitle: '☀️ 여름철 반려동물 건강 관리',
    summerCareDesc: '폭염 속 우리 아이를 지키는 필수 수칙',
    summerHeatstroke: '열사병 예방',
    summerHydration: '수분 및 체온 관리',
    summerTick: '참진드기 · 기생충 예방',
    summerFood: '식중독 예방 & 사료 보관',
    summerEmergency: '🚨 응급 상황 대처법',
    summerWalkTip1: '이른 아침이나 해가 진 저녁 시간대에 산책해요',
    summerWalkTip2: '손등을 바닥에 5초간 대어보고 뜨겁다면 자제',
    summerWalkTip3: '아스팔트 대신 흙길이나 그늘길을 이용',
    summerAC: '에어컨 온도 24~26도 유지, 에어컨 없는 곳에서 생긴 갈증 주의',
    summerCooling: '미지근한 물에 적신 수건으로 복부와 발바닥 닦아주기',
    summerWater: '항상 신선한 물 여러 곳에 배치, 하루 2회 교체',
    summerTickPrevent: '진드기 예방약 정기 사용, 산책 후 몸 전체 확인',
    summerFoodSafety: '남은 사료 바로 치우기, 그릇 매일 세척',
    summerStorage: '사료 건조하고 시원한 곳 보관, 개봉 후 1달 내 소비',
    summerEmergencyTitle: '열사병 의심 증상',
    summerEmergencySymptoms: '헐떡임·구토·비틀거림·침 흘림·의식 저하',
    summerEmergencyAction: '시원한 곳으로 이동 → 미지근한 물로 몸 식히기 → 즉시 병원 이동',
    summerWildMushroom: '야생 버섯 섭취 금지 (독버섯 위험)',
    summerSFTSWarning: 'SFTS(중증열성혈소판감소증후군) 주의',
    summerSFTSDesc: '진드기 매개 바이러스, 예방백신 없음',
    summerSFTSSymptoms: '5~14일 내 고열(38℃ 이상)·식욕부진·구토·설사',
    summerSFTSAction: '야외활동 후 털과 피부 꼼꼼히 확인, 증상 시 즉시 병원',
    fallCareTitle: '🍂 가을철 반려동물 건강 관리',
    fallCareDesc: '계절 변화기 우리 아이를 위한 필수 수칙',
    fallTempChange: '일교차 대응',
    fallShedding: '털갈이 관리',
    fallAllergy: '알레르기 주의',
    fallEyeCare: '눈 건강 관리',
    fallHydration: '수분 섭취',
    fallWalk: '산책 관리',
    fallTip1: '일교차가 큰 시간대에는 반려동물 옷차림 조절',
    fallTip2: '햇볕이 따뜻할 때는 얇은 옷, 밤에는 보온 의류 착용',
    fallTip3: '매일 빗질로 털갈이 관리, 하루 1~2회 빗질',
    fallTip4: '집 먼지 제거 및 침구 청소로 알레르기 비염 예방',
    fallTip5: '눈물 자국 제거 및 인공눈물로 건조 예방',
    fallTip6: '여름처럼 많이 마시지 않지만 수분 섭취 꾸준히 유지',
    fallTip7: '서늘한 아침·저녁 시간대에 산책 즐기기',
    fallTip8: '풀이 마르면서 들개풀 등 알레르기 유발 식물 주의',
    autumnCareTitle: '🍂 가을철 반려동물 건강 관리',
    autumnCareDesc: '환절기 일교차와 가을해충까지, 우리 아이 필수 수칙',
    autumnTempChange: '일교차 대응',
    autumnWalkTip1: '서늘한 아침·저녁 산책 시 반려동물 옷차림 조절',
    autumnWalkTip2: '일교차가 클 땐 실내 온도를 급격히 바꾸지 않기',
    autumnClothing: '단모종·시니어는 보온 의류로 체온 유지',
    autumnParasites: '가을해충 주의 (모기·진드기)',
    autumnTickPrevent: '산책 전 반려견용 모기·진드기 기피제 사용 (성분 이카리딘 15% 기준 최대 5시간 효과)',
    autumnFleaPrevent: '성묘·벌초·나들이 후에는 털과 피부를 꼼꼼히 확인',
    autumnRegularCheck: '월 1회 구충제 투여는 가을에도 계속 — 가을해충이 더 활발',
    autumnSkin: '피부·털 관리',
    autumnDrySkin: '건조해지는 환절기에는 보습 관리와 목욕 후 완전 건조',
    autumnBrushing: '환절기 털갈이 시즌, 매일 빗질로 죽은 털 제거',
    autumnSkinCare: '잦은 긁음·붉은 반점이 보이면 피부병 조기 진료',
    autumnAllergy: '환절기 알레르기 주의 — 재채기·귀를 계속 긁거나 핥으면 알레르기 검사 고려',
    autumnNose: '맑은 콧물은 환절기 일시적일 수 있으나, 노랗고 끈적한 콧물이 2~3일 이상 지속되면 만성 비염 가능성 — CT·세균검사 등 정밀 진단 권장',
    autumnFood: '식단 관리',
    autumnNutrition: '환절기 면역력을 위해 균형 잡힌 사료·영양제 유지',
    autumnWeightManage: '활동량 변화에 맞춰 급여량 조절로 비만 예방',
    autumnApples: '사과 등 과일은 씨앗·심 제거 후 소량만 급여',
    autumnCompleteFeed: "'반려동물 완전사료' 표시 확인 — 영양 기준 충족 제품만 표시 허용(2026 개정), 라벨의 '연어 함유'는 3%, '연어'는 25% 함유 의미",
    autumnHygiene: '반려동물 항생제 내성균이 사람에게 전파될 수 있어 접촉 후 손 씻기, 특히 아이·노인 가정은 위생 수칙 준수',
    autumnExercise: '산책·놀이',
    autumnWalkTime: '쾌적한 가을 날씨, 산책 시간을 조금씩 늘려 체력 충전',
    autumnIndoorPlay: '비 오는 날엔 실내 노즈워크·퍼즐 장난감으로 대체',
    autumnWeatherCheck: '미세먼지·꽃가루 농도가 높은 날은 실내 활동 권장',
    autumnEmergency: '응급 상황 대비',
    autumnEmergencyTitle: '진드기·SFTS 의심 증상',
    autumnEmergencySymptoms: '고열, 무기력, 식욕 부진, 점상 출혈',
    autumnEmergencyAction: '야외활동 후 털과 피부 꼼꼼히 확인, 증상 시 즉시 병원',
    dailyMission: {
      title: '🐾 오늘의 반려동물 미션',
      desc: '오늘 우리 아이에게 필요한 오행 기운을 채워주세요!',
      completed: '오늘의 미션 완료!',
      congrats: '축하합니다! 3개 미션을 모두 완료했어요!',
      progress: '미션 달성도',
      badge: '오늘의 우수 반려인',
      missions: {
        wood: {
          dog: { title: '숲속 탐험', desc: '나무가 많은 곳에서 자연의 냄새를 맡게 해주세요.', time: '15분' },
          cat: { title: '캣그라스 파티', desc: '싱싱한 캣그라스를 제공하거나 풀 냄새를 맡게 해주세요.', time: '10분' }
        },
        fire: {
          dog: { title: '에너지 발산', desc: '심박수가 올라갈 정도로 신나게 뛰어놀아 주세요.', time: '20분' },
          cat: { title: '우다다 타임', desc: '낚시대로 사냥 본능을 자극해 힘차게 움직이게 해주세요.', time: '15분' }
        },
        earth: {
          dog: { title: '흙길 산책', desc: '아스팔트 대신 폭신한 흙이나 잔디를 밟게 해주세요.', time: '15분' },
          cat: { title: '아늑한 휴식', desc: '가장 좋아하는 박스나 구석진 곳에서 푹 쉬게 해주세요.', time: '30분' }
        },
        metal: {
          dog: { title: '영리한 훈련', desc: '새로운 개인기를 가르치거나 앉아/기다려 복습을 해보세요.', time: '10분' },
          cat: { title: '완벽한 그루밍', desc: '평소보다 더 꼼꼼하고 부드럽게 빗질을 해주세요.', time: '15분' }
        },
        water: {
          dog: { title: '수분 충전', desc: '신선한 물로 교체해주고 충분히 마실 수 있게 도와주세요.', time: '5분' },
          cat: { title: '흐르는 물 관찰', desc: '음수대나 수도꼭지 물소리로 호기심을 자극해보세요.', time: '10분' }
        }
      }
    }
  },
  en: {
    appTitle: '🐾 PetLife',
    heroEmoji: '🐾',
    heroTitle: 'Pet Saju Reading',
    heroDesc: 'Discover your dog or cat\'s Four Pillars of Destiny (Saju) in a fun way!',
    badges: ['Saju Analysis', 'Five Elements', 'Personality', 'Fortune'],
    petType: 'Pet Type',
    dog: 'Dog',
    cat: 'Cat',
    nameLabel: 'Name',
    namePlaceholder: 'e.g. Coco',
    catPlaceholder: 'e.g. Luna',
    breedLabel: 'Breed',
    breedDefault: 'Select breed',
    birthLabel: 'Date of Birth',
    hourLabel: 'Time of Birth',
    submitBtn: 'Read Saju',
    fourPillars: 'Four Pillars (四柱)',
    year: 'Year (年)',
    month: 'Month (月)',
    day: 'Day (日)',
    hour: 'Hour (時)',
    ilju: 'Day Master',
    personality: 'Personality Analysis',
    traits: 'Traits',
    fiveElements: 'Five Elements (五行)',
    elementRadarTitle: 'Five Elements Balance Guide',
    elementRadarDesc: 'Visually check your pet\'s natural energy balance.',
    missingElements: 'Missing Elements',
    walkStyle: 'Walk Style',
    catPlayStyle: 'Play & Activity Style',
    foodPref: 'Diet & Food Tips',
    healthTip: 'Health Care Tips',
    compatible: 'Compatible Day Masters',
    todayLuck: 'Today\'s Fortune',
    todayIlju: 'Today\'s Day Master',
    monthlyLuck: 'Monthly Fortune',
    monthlyEnergy: "This Month's Energy",
    weeklyHoroscope: 'Weekly Pet Horoscope',
    weeklyHoroscopeDesc: "Check your pet's 7-day fortune for this week!",
    weeklyScore: 'Fortune Score',
    todayHighlight: 'Today',
    careTips: 'Personalized Care Tips',

    shareBtn: 'Share Result',
    printBtn: 'Save/Print',
    imageBtn: 'Save Image',
    saving: 'Saving...',
    resetBtn: 'Start Over',
    copied: 'Copied to clipboard',
    historyTitle: 'Saved Saju Records',
    historyDesc: 'Up to 20 records saved automatically',
    compatTitle: 'Pet Compatibility',
    compatDesc: 'Check compatibility with another pet!',
    compatNameLabel: 'Name',
    compatBirthLabel: 'Date of Birth',
    compatHourLabel: 'Time of Birth',
    compatBtn: 'Check Compatibility',
    compatChecking: 'Checking...',
    compatRetry: 'Try Again',
    missingWood: 'Loves nature walks! Take them to the park often.',
    missingFire: 'Enjoys warm sunshine. Give plenty of sunbathing time.',
    missingEarth: 'A stable environment matters. Make home cozy.',
    missingMetal: 'Grooming is important. Regular grooming recommended.',
    missingWater: 'May enjoy water activities. Try swimming!',
    balanced: 'All five elements are balanced — a very well-rounded pet!',
    footer: '© 2026 PetLife — Happy life with your pet 🐶',
    darkMode: 'Light Mode',
    lightMode: 'Dark Mode',
    records: 'Records',
    close: 'Close',
    yearSuffix: '',
    monthSuffix: '',
    daySuffix: '',
    hourSuffix: '',
    zodiacSuffix: '',
    of: '\'s Saju',
    resultOf: '\'s Saju Result',
    saveProfile: 'Save my pet profile',
    savedProfiles: 'Saved Profiles',
    deleteProfile: 'Delete',
    petTypeTitle: 'Pet Personality Type',
    petTypeDesc: 'Personality type analyzed based on the Five Elements distribution.',
    petMBTITitle: '🐾 Pet MBTI Personality',
    petMBTISubtitle: 'Your pet\'s personality analyzed through saju elements',
    petMBTIStrengths: 'Key Strengths',
    petMBTIGrowth: 'Growth Points',
    petMBTIDisclaimer: 'This result is a fun personality type based on your pet\'s saju energy.',
    personalityE: 'Active (Fire)',
    personalityI: 'Calm (Water)',
    personalityS: 'Practical (Earth)',
    personalityN: 'Imaginative (Wood)',
    personalityT: 'Logical (Metal)',
    personalityF: 'Emotional (Fire)',
    personalityJ: 'Planned (Yang)',
    personalityP: 'Spontaneous (Yin)',
    luckyNameTitle: 'Lucky Name Recommendation',
    luckyNameDesc: 'Names that bring good energy to your pet',
    refreshBtn: 'Refresh',
    meaningLabel: 'Meaning',
    tarotTitle: '🐾 Daily Tarot',
    tarotInstruction: 'Choose a card that calls to you',
    tarotRedraw: 'Draw Again',
    tarotSharePrefix: 'Tarot Card',
    fengshuiTitle: '🐾 Lucky Direction & Interior',
    fengshuiDesc: 'Tips for space arrangement to maximize your pet\'s energy.',
    fengshuiBedDir: 'Bed Direction',
    fengshuiFoodDir: 'Feeding Area Direction',
    fengshuiColors: 'Lucky Colors',
    fengshuiInterior: 'Interior Point',
    fengshuiTipsTitle: 'Pet Specific Tips',
    fengshuiBoostTitle: 'Boosting Missing Energy',
    fengshuiBoostDesc: 'Filling in missing elements can lead to a healthier and happier life.',
    fengshuiCompassE: 'E',
    fengshuiCompassW: 'W',
    fengshuiCompassS: 'S',
    fengshuiCompassN: 'N',
    fengshuiCompassC: 'C',
    wishCouponTitle: '🐾 Pet Wish Coupons',
    wishCouponDesc: 'Special coupons tailored to your pet\'s personality!',
    wishCouponRegenerate: 'Draw New Coupons',
    wishCouponUsed: 'Used',
    wishCouponUnused: 'Unused',
    wishCouponDate: 'Date Used',
    wishCouponPlaceholder: 'YYYY-MM-DD',
    gachaTitle: '🐾 Daily Fortune Capsule',
    gachaDesc: 'Draw a capsule to see your pet\'s fortune for today!',
    gachaBtn: 'Draw Capsule',
    gachaDrawing: 'Drawing capsule...',
    gachaResultTitle: 'Today\'s Fortune Card',
    gachaAffection: 'Affection',
    gachaHealth: 'Health',
    gachaPlay: 'Play',
    gachaAppetite: 'Appetite',
    gachaDailyLimit: 'You can only draw once a day.',
    moodTrackerTitle: 'Today\'s Mood',
    moodTrackerDesc: 'Log your pet\'s mood',
    moodStreak: '{n}-day streak',
    moodHappy: 'Happy',
    moodSleepy: 'Sleepy',
    moodPlayful: 'Playful',
    moodHungry: 'Hungry',
    moodAnxious: 'Anxious',
    moodNotLogged: 'Not logged',
    dailyMission: {
      title: '🐾 Daily Pet Mission',
      desc: 'Fill up your pet\'s energy with today\'s missions!',
      completed: 'Today\'s Mission Complete!',
      congrats: 'Congratulations! All 3 missions completed!',
      progress: 'Mission Progress',
      badge: 'Best Pet Parent',
      missions: {
        wood: {
          dog: { title: 'Forest Adventure', desc: 'Let your pet smell nature where there are many trees.', time: '15m' },
          cat: { title: 'Catgrass Party', desc: 'Provide fresh catgrass or let them smell some plants.', time: '10m' }
        },
        fire: {
          dog: { title: 'Burn Energy', desc: 'Play actively until their heart rate goes up.', time: '20m' },
          cat: { title: 'Zoomie Time', desc: 'Stimulate their hunting instinct with a wand toy.', time: '15m' }
        },
        earth: {
          dog: { title: 'Soil Walk', desc: 'Let them walk on soft soil or grass instead of asphalt.', time: '15m' },
          cat: { title: 'Cozy Rest', desc: 'Let them rest in their favorite box or a quiet corner.', time: '30m' }
        },
        metal: {
          dog: { title: 'Smart Training', desc: 'Teach a new trick or review basic commands.', time: '10m' },
          cat: { title: 'Perfect Grooming', desc: 'Brush them more thoroughly and gently than usual.', time: '15m' }
        },
        water: {
          dog: { title: 'Hydration', desc: 'Replace with fresh water and help them drink enough.', time: '5m' },
          cat: { title: 'Flowing Water', desc: 'Stimulate curiosity with a water fountain or tap.', time: '10m' }
        }
      }
    },
    fallCareTitle: '🍂 Fall Pet Health Care',
    fallCareDesc: 'Essential care during seasonal transition',
    fallTempChange: 'Temperature Fluctuation',
    fallShedding: 'Shedding Management',
    fallAllergy: 'Allergy Awareness',
    fallEyeCare: 'Eye Health',
    fallHydration: 'Hydration',
    fallWalk: 'Walk Management',
    fallTip1: 'Adjust clothing for temperature fluctuations',
    fallTip2: 'Light clothes when sunny, warm layers at night',
    fallTip3: 'Daily brushing for shedding management, 1-2 times/day',
    fallTip4: 'Dust removal and bedding cleaning to prevent allergic rhinitis',
    fallTip5: 'Remove tear stains and use artificial tears for dry eyes',
    fallTip6: 'Maintain consistent hydration even without summer thirst',
    fallTip7: 'Enjoy walks during cool morning and evening hours',
    fallTip8: 'Watch for allergy-triggering plants like mugwort as grass dries',
    autumnCareTitle: '🍂 Autumn Pet Health Care',
    autumnCareDesc: 'Essential tips for temperature swings and autumn pests',
    autumnTempChange: 'Temperature Swings',
    autumnWalkTip1: 'Adjust clothing for cool morning and evening walks',
    autumnWalkTip2: 'Avoid sudden indoor temperature changes on high-swing days',
    autumnClothing: 'Use warm clothing for short-haired or senior pets',
    autumnParasites: 'Autumn Pests (Mosquitoes & Ticks)',
    autumnTickPrevent: 'Use repellent before walks (picaridin 15% offers up to 5 hours of protection)',
    autumnFleaPrevent: 'Check fur and skin thoroughly after hikes or outings',
    autumnRegularCheck: 'Keep monthly deworming through autumn — pests stay active',
    autumnSkin: 'Skin & Coat',
    autumnDrySkin: 'Moisturize as air dries; dry fully after baths',
    autumnBrushing: 'Daily brushing during seasonal shedding',
    autumnSkinCare: 'See a vet early for frequent scratching or red spots',
    autumnAllergy: 'Watch for seasonal allergies — sneezing or constant ear scratching warrants an allergy check',
    autumnNose: 'Clear runny nose may be seasonal, but yellow sticky discharge lasting 2-3+ days suggests chronic rhinitis — CT and bacterial testing recommended',
    autumnFood: 'Diet',
    autumnNutrition: 'Keep balanced food and supplements for seasonal immunity',
    autumnWeightManage: 'Adjust portions to match changing activity levels',
    autumnApples: 'Fruits like apples: remove seeds and core, feed small amounts',
    autumnCompleteFeed: "Look for the 'complete pet food' label — only products meeting nutrient standards may use it (2026 revision); 'salmon with' means 3% vs 'salmon' 25% content",
    autumnHygiene: 'Antimicrobial-resistant bacteria can pass from pets to people — wash hands after contact, especially in homes with children or seniors',
    autumnExercise: 'Walks & Play',
    autumnWalkTime: 'Gradually extend walk time in pleasant fall weather',
    autumnIndoorPlay: 'On rainy days, swap in nose work and puzzle toys',
    autumnWeatherCheck: 'Prefer indoor activities on high fine-dust or pollen days',
    autumnEmergency: 'Emergency Signs',
    autumnEmergencyTitle: 'Suspected tick-borne SFTS',
    autumnEmergencySymptoms: 'Fever, lethargy, loss of appetite, petechiae',
    autumnEmergencyAction: 'Check fur and skin after outdoor activity; see a vet immediately if symptomatic'
  }
}

const STORAGE_KEY = 'petlife_lang'
const isBrowser = typeof window !== 'undefined' && typeof localStorage !== 'undefined'
const currentLang = ref(isBrowser ? loadLang() : 'ko')

function loadLang() {
  if (!isBrowser) return 'ko'
  const saved = localStorage.getItem(STORAGE_KEY)
  if (saved && messages[saved]) return saved
  const nav = navigator.language || 'ko'
  return nav.startsWith('ko') ? 'ko' : 'en'
}

function setLang(lang) {
  currentLang.value = lang
  if (isBrowser) localStorage.setItem(STORAGE_KEY, lang)
}

function toggleLang() {
  setLang(currentLang.value === 'ko' ? 'en' : 'ko')
}

function t(key) {
  return messages[currentLang.value]?.[key] || messages.ko[key] || key
}

export { currentLang, setLang, toggleLang, t }
